...
//alert("This system contains three graphs. Two of these graphs, except radial scatter plot, only support zooming and panning. However, radial scatter plot has track volume control at the top left corner, and the center circles serve as control buttons of the audio player. Click on notes in the plot would change other views. Please use zooming and panning to find the interesting spot. ")
...
var reMap = function(oldValue) {
  ...
  var colorScheme = ["#b7fbff", "#dbe9b7", "#f7b633", "#ffe0a3", "#ffa1ac"];
  var tracksAngularLinearScales = [];
  var trackPolarRadiusScales = [];
  var lineChatPitchLinearScales = [];
  var lineChatTicksLinearScales = [];
  var lineChatRadiusLinearScales = [];
  var radiusScale;
...
*/
  d3.csv("./data/track_info_ver5.csv", function(data){
    ...
    while (trackIndex < numTracks){
        ...
        lineChatTicksLinearScales[trackIndex] = d3.scaleLinear().domain([d3.min(data, function(d) { if(d["Track"] - 1 == trackIndex) {return +d["Start"]}; }), d3.max(data, function(d) { if(d["Track"] - 1 == trackIndex) {return +d["Start"]}; })]).range([0, 100]);
        lineChatPitchLinearScales[trackIndex] = d3.scaleLinear().domain([d3.min(data, function(d) { if(d["Track"] - 1 == trackIndex) {return +d["Pitch"]}; }) , d3.max(data, function(d) { if(d["Track"] - 1 == trackIndex) {return +d["Pitch"]}; })]).range([-3.5, 3.5]);
        lineChatRadiusLinearScales[trackIndex] = d3.scaleLinear().domain([d3.min(data, function(d) { return +d["Duration"]; }), d3.max(data, function(d) { return +d["Duration"]; })]).range([5, 10]);
    ...
    currentInstrument = data[0]["Instrument"];
    ...
    data.forEach(rawNote => {
        processedData.push(Array.from(Object.create([
          ...
          rawNote["Instrument"],
          +rawNote["Track"],
          colorScheme[+rawNote["Track"] - 1],rawNote["Indx"]
        ])));

        lineChatData[+rawNote["Track"] - 1].push(Array.from(Object.create([
          +rawNote["Track"],
          lineChatTicksLinearScales[((+rawNote["Track"]) - 1)](+rawNote["Start"]),
          lineChatPitchLinearScales[(+rawNote["Track"]) - 1](+rawNote["Pitch"]),
          rawNote["Octave"],
          lineChatRadiusLinearScales[((+rawNote["Track"]) - 1)](+rawNote["Duration"]),
          colorScheme[+rawNote["Track"] - 1],rawNote["Indx"]
        ])));
        //console.log();
    ...
    d3.select(".notes").selectAll("note")
      ...
      .append("circle")
      .attr("class", "note")
      .attr("track", function(d){ return d[5]; })
      .attr("indexes", function(d){ return d[7]-1; })
      .attr("notename", function(d){ return d[3]; })
      ...
      .style("fill", function(d){
        ...
        //console.log(d);
        d3.selectAll('.note').style("opacity", 0.1);

        currentSelectedTrack = d[5];
        currentInstrument = d[4];
        updateTrack();
        d3.selectAll('.note').filter(function(d){
        return d[5] === currentSelectedTrack;}).style("opacity", 1);
        var flag = true;
      }).on("mouseover", function(d) {
        d3.selectAll('.note').style("opacity", 0.1);
        var noteid = d3.select(this).attr("notename");
        d3.selectAll('.note').filter(function(d){
        return d[3] === noteid;}).style("opacity", 0.8);
        var tooltip = d3.select(".tooltipContainer");
        tooltip.style("left", (d3.event.pageX - 50) + "px");
        tooltip.style("top", (d3.event.pageY - 60) + "px");
        d3.select(".tooltipMapText").html("<p>" +"<strong><font size = 2>"+ d[4] +"</font>"+ " | " +"<font size = 2 >" + d[3] +"</font></strong>"+ "</p>");
      ...
      .on("mouseleave", function(d) {
        if (d3.select(this).style("opacity")!=1) {
          d3.selectAll('.note').style("opacity", 0.8);
        }
          var tooltip = d3.select(".tooltipContainer");
...
*/
  ...
  function initPlayerControl(){
    //#fcd307
    var control = d3.select(".notes").append('g').attr("id", "playerControl");
    control.append("circle")
      ...
      .on("mouseleave", function(d){
        d3.select(".notes").selectAll('.button')
      ...
      var i = 0;
        while(i < numTracks) {
          control.append("circle")
            ...
            .attr("transform", "translate(-300, " + (-300 + i * 15) + ")")
            .attr("id", function(d){ return "track" + i; })
            .on("click", function(d) {
            ...
            .on('mouseout', function(d){
              var x = d3.select(this).attr("id");
              InstrumentRemove(x);
            // d3.select(".notes").selectAll('.label').remove();
            })
    ...
    // console.log(i)
     if (i == "track2"){
      d3.select(".notes")
                ...
                .attr("class", "label")
                .attr("x", -330)
                .attr("y", -264).style("font-family", "Book Antiqua")
                .attr("font-weight", "bold")
      ...
      d3.select(".notes")
                ...
                .attr("class", "label")
                .attr("x", -330)
                .attr("y", -278).style("font-family", "Book Antiqua")
                .attr("font-weight", "bold")
      ...
      d3.select(".notes")
                ...
                .attr("class", "label")
                .attr("x", -330)
                .attr("y", -292).style("font-family", "Book Antiqua")
                .attr("font-weight", "bold")
                .attr("transform", "translate(40,0)")
                .style("opacity",0)
                .transition().duration(100)
  ...
  function InstrumentRemove(i){

    if (i == "track2"){
      d3.select(".notes").selectAll('.label')
  ...
  function updateView() {
    ...
    var lineChatSliderXPos = getCurrentPlaybackTime() / globalDuration * 19000;
    //console.log(lineChatSliderXPos);
    updateSliderline(lineChatSliderXPos);
  }
...
/*
    ...
    var gX = group.append("g")
        .attr("class", "x axis")
        .attr("transform", "translate(0," + height + ")")
        .call(xAxis);
    var gY = group.append("g")
    ...
    group.append("g")
      .attr("class", "charts")
      .append("path")
        .datum(lineChatData[currentSelectedTrack - 1])
        .attr("class", "line")
    ...
    group.append("g")
      .attr("class", "charts")
      .selectAll(".note")
      .data(lineChatData[currentSelectedTrack - 1]).enter()
        .append("circle")
        .attr("class", "note")
        .attr("indexes", function(d){ return d[6]-1; })
    ...
    group.append("g")
      ...
      .data(lineChatData[currentSelectedTrack - 1]).enter()
        ...
        .attr("y", function(d){return y(d[2]) + 20; })
  ...
  function updateSliderline(position){
...
var fileName = "./data/track_" + (currentSelectedTrack - 1) + ".json";
...
/*
  ...
  function zoomed() {
      svg.selectAll("#arcDiagramSub")
          .attr("transform", d3.event.transform);
      //d3.selectAll('.line').style("stroke-width", 2/d3.event.transform.k);
  }
*/
  var svg = d3.select("body").append("svg")
      .attr("viewBox", "700 0 1200 700")
      .attr("width", width)
      .attr("height", height)
  ...
  path.enter()
      ...
      .attr("target", function(d,i){ return +nodeDisplayX(nodes[d.target]);})
      .attr("source", function(d,i){ return +nodeDisplayX(nodes[d.source]);})
      ...
      d3.select(this).style('fill', function(d,i) {return colorScheme[currentSelectedTrack - 1];}).style('opacity', 1);
      var s_v = +d3.select(this).attr("source");
      var t_v = +d3.select(this).attr("target");
      var w_v = +d3.select(this).attr("width");
      // d3.selectAll("path[source='id']").style('fill','red');
      ...
      d3.selectAll('.nodes')
        .filter(function(){
                  return (d3.select(this).attr("cx") == (s_v+i*11)) || (d3.select(this).attr("cx") ==( t_v+i*11));
       }).style('opacity', 1);
        }
      for (var i = 0; i < w_v; i++) {
      var indtemp = d3.selectAll('.nodes')
        .filter(function(){
                  return (d3.select(this).attr("cx") == (s_v+i*11));
       }).attr('realndx');
        matching.push(indtemp);
      }
      for (var i = 0; i < w_v; i++) {
      var indtemp = d3.selectAll('.nodes')
        .filter(function(){
                  return (d3.select(this).attr("cx") == (t_v+i*11));
       }).attr('realndx');
        matching.push(indtemp);
      }
    ...
    .on('mouseout', function (d) {
      d3.select(".notes").selectAll(".note").style('opacity', 0.8);
      d3.selectAll(".charts").selectAll(".note").style('opacity', 1).attr("r", function(d){return d[4];}).style("stroke-width", 0);
      d3.selectAll('.links').style('fill', '#ffffff');
      d3.selectAll('.links').style('opacity', 1);
      d3.selectAll('.nodes').style('opacity', 1);});


function match(x){
  d3.select(".notes").selectAll(".note").style('opacity', 0.1);
  // console.log(currentSelectedTrack);
  for (var i = 0; i < x.length; i++) {
   d3.select(".notes").selectAll(".note")
        .filter(function(){
                  return (d3.select(this).attr("track") == currentSelectedTrack && d3.select(this).attr("indexes") == x[i]);
       }).style('opacity', 1);
  }
  d3.selectAll(".charts").selectAll(".note").style('opacity', 0.2);
  for (var i = 0; i < x.length; i++) {
  d3.selectAll(".charts").selectAll(".note")
        .filter(function(){
                  return (d3.select(this).attr("indexes") == x[i]);
       }).style('opacity', 1).attr("r", function(d){return d[4]*2;}).style("stroke", "white").style("stroke-width", 3);
  }
  // d3.selectAll(".charts").selectAll(".line").style('opacity', 0.1);
}
  ...
  circle.enter()
      ...
      .attr("cy", nodeY)
      // .attr("index", function(d,i) {return nodeDisplayX(d);})
      .style('opacity', 1)
      .attr("realndx", function(d,i) {
        return (nodeDisplayX(d)-20)/11;})
      .attr("cx", function(d,i) {return nodeDisplayX(d);})
      .attr("r", function(d,i) {return mapRange(d.value, nodeValMin, nodeValMax, 2.5, 13);})
      .attr("fill", function(d,i) {return colorScheme[currentSelectedTrack - 1];})
      ...
      // d3.selectAll('.nodes').filter(function (d) { return nodesToHighlight.indexOf(d) >= 0; })
      //   .style('opacity', 0.2);
      var idx = d3.select(this).attr("cx");
      // d3.selectAll("path[source='id']").style('fill','red');
      d3.selectAll('.links').style('opacity', 0);
      d3.selectAll('.links')
        .filter(function(){
                  // return (d3.select(this).attr("source") == idx);
        return (d3.select(this).attr("source") == idx) || (d3.select(this).attr("target") == idx);
       }).style('opacity', 1);
      d3.select(this).style('opacity', 1);})
...
function updateArcDiagram() {
  d3.select("#arcDiagram").remove();
  initArcDiagram();
}
},{"d3v4":2,"jquery":3}],2:[function(require,module,exports){
